    ...
    where TListViewModel : IEntityDto<TKey>
{
    ...
    protected string CurrentFiltering = default!;
    ...
    {
        if (GetListInput is IFiltering filteredResultRequestInput)
        {
            filteredResultRequestInput.Filtering = CurrentFiltering;
        }
    ...
    protected virtual async Task OnDataGridReadAsync(DataGridReadDataEventArgs<TListViewModel> e)
    {
        CurrentSorting = e.Columns
            ...
            .JoinAsString(",");
        CurrentFiltering = e.Columns.Where(c => c.SearchValue != null && c.FilterMethod.HasValue)
            .Select(s => CreateStringExpression(s.Field, s.FilterMethod, s.ColumnType, s.SearchValue)).JoinAsString(" and ");
        CurrentPage = e.Page;
    ...
    string CreateStringExpression(string fieldName, DataGridColumnFilterMethod? method, DataGridColumnType columnType, object val)
    {
        if (columnType == DataGridColumnType.Command)
        {
            return "";
        }
        var sc = "";
        if (columnType == DataGridColumnType.Text)
        {
            sc = "\"";
        }
        if (columnType == DataGridColumnType.Date && val != null)
        {
            sc = "\"";
            var valStr1 = ((DateTime)val).ToString("yyyy-MM-dd") + " 00:00:00";
            var valStr2 = ((DateTime)val).AddDays(1).ToString("yyyy-MM-dd") + " 00:00:00";
            var result = method switch
            {
                DataGridColumnFilterMethod.Contains => $"{fieldName} >= {sc}{valStr1}{sc} and {fieldName} <= {sc}{valStr2}{sc}",
                DataGridColumnFilterMethod.Equals => $"{fieldName} >= {sc}{valStr1}{sc} and {fieldName} <= {sc}{valStr2}{sc}",
                DataGridColumnFilterMethod.NotEquals => $"{fieldName} < {sc}{valStr1}{sc} and {fieldName} > {sc}{valStr2}{sc}",
                DataGridColumnFilterMethod.LessThan => $"{fieldName} < {sc}{valStr1}{sc}",
                DataGridColumnFilterMethod.LessThanOrEqual => $"{fieldName} < {sc}{valStr2}{sc}",
                DataGridColumnFilterMethod.GreaterThan => $"{fieldName} >= {sc}{valStr2}{sc}",
                DataGridColumnFilterMethod.GreaterThanOrEqual => $"{fieldName} >= {sc}{valStr1}{sc}",
        ...
        else
        {
            ...
            var result = method switch
            {
                DataGridColumnFilterMethod.Contains => $"{fieldName}.Contains({sc}{valStr}{sc})",
                DataGridColumnFilterMethod.StartsWith => $"{fieldName}.StartsWith({sc}{valStr}{sc})",
                DataGridColumnFilterMethod.EndsWith => $"{fieldName}.EndsWith({sc}{valStr}{sc})",
                DataGridColumnFilterMethod.Equals => $"{fieldName} == {sc}{valStr}{sc}",
                DataGridColumnFilterMethod.NotEquals => $"{fieldName} != {sc}{valStr}{sc}",
                DataGridColumnFilterMethod.LessThan => $"{fieldName} < {sc}{valStr}{sc}",
                DataGridColumnFilterMethod.LessThanOrEqual => $"{fieldName} <= {sc}{valStr}{sc}",
                DataGridColumnFilterMethod.GreaterThan => $"{fieldName} > {sc}{valStr}{sc}",
                DataGridColumnFilterMethod.GreaterThanOrEqual => $"{fieldName} >= {sc}{valStr}{sc}",
                _ => throw new ArgumentOutOfRangeException(nameof(method), method, null)
            };
            return result;
        }
