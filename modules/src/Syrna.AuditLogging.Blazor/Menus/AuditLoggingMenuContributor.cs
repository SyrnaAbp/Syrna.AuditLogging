...
public class AuditLoggingMenuContributor : IMenuContributor
{
    ...
    private async Task ConfigureMainMenuAsync(MenuConfigurationContext context)
    {
        if (await context.IsGrantedAsync(AuditLoggingPermissions.AuditLogs.Default))
        {
            ...
            context.Menu.GetAdministration().AddItem(new ApplicationMenuItem(AuditLoggingMenus.Prefix, displayName: l["AuditLogging"], "~/AuditLogging/AuditLogs/AuditLog", icon: "fa fa-globe"));
        }
    }
}